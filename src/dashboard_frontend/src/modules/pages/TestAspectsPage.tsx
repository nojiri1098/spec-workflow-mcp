            ...
            <div className="flex items-center gap-2">
              ...
              {isArchived && (
                <span className="inline-flex items-center px-2 py-1 text-xs font-medium bg-orange-100 text-orange-800 dark:bg-orange-900/20 dark:text-orange-400 rounded-full">
                  ...
                  {t('testAspectsPage.modal.archivedBadge')}
                </span>
              )}
            </div>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1 hidden sm:block">
              {isArchived ? `${t('testAspectsPage.modal.archivedNotice')} • ` : ''}{t('common.lastModified', { date: formatDate(spec.lastModified, t) })}
            </p>
          </div>
          <button
            onClick={handleClose}
            className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-200 transition-colors p-2 -m-2 ml-4"
            aria-label={t('testAspectsPage.modal.closeAria')}
          >
...
function Content() {
  ...
  const specSortOptions = [
    {
      id: 'default',
      label: t('testAspectsPage.sort.defaultOrder'),
      description: t('testAspectsPage.sort.defaultOrderDesc'),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      id: 'name',
      label: t('testAspectsPage.sort.name'),
      description: t('testAspectsPage.sort.nameDesc'),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      id: 'progress',
      label: t('testAspectsPage.sort.progress'),
      description: t('testAspectsPage.sort.progressDesc'),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    {
      id: 'lastModified',
      label: t('testAspectsPage.sort.lastModified'),
      description: t('testAspectsPage.sort.lastModifiedDesc'),
      icon: (
        <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" />
        </svg>
  ...
  return (
    <div className="grid gap-4">
      <div className="bg-white dark:bg-gray-800 shadow rounded-lg p-4 sm:p-6">
        <div className="mb-4">
          <h2 className="text-lg sm:text-xl font-semibold text-gray-900 dark:text-white">{t('testAspectsPage.header.title')}</h2>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            {activeTab === 'active'
              ? t('testAspectsPage.header.subtitle.active')
              : t('testAspectsPage.header.subtitle.archived')
            }
          </p>
        </div>
        ...
        <div>
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 py-2">
            <nav className="flex space-x-8">
              ...
              >
                {t('testAspectsPage.tabs.active')} ({specs.length})
              </button>
              <button
              ...
              >
                {t('testAspectsPage.tabs.archived')} ({archivedSpecs.length})
              </button>
            </nav>
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
              <input
                ...
                placeholder={activeTab === 'active' ? t('testAspectsPage.search.placeholder.active') : t('testAspectsPage.search.placeholder.archived')}
                value={query}
            ...
            <thead className="bg-gray-50 dark:bg-gray-900">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('testAspectsPage.table.name')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('testAspectsPage.table.progress')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('testAspectsPage.table.lastModified')}
                </th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  {t('testAspectsPage.table.actions')}
                </th>
              </tr>
            </thead>
        ...
        {filtered.length === 0 && (
          <div className="text-center py-12 mt-8 border-t border-gray-200 dark:border-gray-700">
            ...
            <p className="text-lg font-medium text-gray-900 dark:text-white mb-2">
              {query ? t('testAspectsPage.empty.noResults') : t('testAspectsPage.empty.title')}
            </p>
            <p className="text-sm text-gray-500 dark:text-gray-400">
              {query ? t('testAspectsPage.empty.noResultsDescription') : t('testAspectsPage.empty.description')}
            </p>
          </div>
        )}
      </div>
...
export function TestAspectsPage() {
  const { initial } = useWs();
  return (
